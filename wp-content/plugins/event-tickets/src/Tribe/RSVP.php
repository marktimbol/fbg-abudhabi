...
class Tribe__Tickets__RSVP extends Tribe__Tickets__Tickets {
	...
	public function save_ticket( $event_id, $ticket, $raw_data = array() ) {
		...
		update_post_meta( $ticket->ID, 'member_type', $ticket->member_type );
	...
	protected function get_tickets( $event_id ) {
		...
		foreach ( $ticket_ids as $post ) {
			...
			} else if(get_user_meta($userid,'paid_unpaid',true)==get_post_meta($post, 'member_type', true)) {
				$tickets[] = $this->get_ticket( $event_id, $post );
	...
	public function get_ticket( $event_id, $ticket_id ) {
		...
		$return->member_type	= get_post_meta( $ticket_id, 'member_type', true );
