...
if(ICL_LANGUAGE_CODE=='fr') {
    $strEvent = "Inscrivez-vous à cet événement";
    ?><h2 class="tribe-events-tickets-title"><?=htmlentities($strEvent, ENT_COMPAT,'ISO-8859-15');?></h2><?
}
else{
    ?><h2 class="tribe-events-tickets-title">Register for this event</h2><?
}
?><form action="" class="tribe-tickets-rsvp cart 123<?php echo esc_attr( $messages_class ); ?>" method="post" enctype='multipart/form-data'>
		...
		<?php if ( $is_there_any_product_to_sell ): ?>
			...
			<tr>
				<td colspan="4" class="add-to-cart">
					<?php if ( $must_login ): ?>
						<a href="<?php echo Tribe__Tickets__Tickets::get_login_url(); ?>" class="tribe-events-button tribe-no-param" style="background:#00abef !important;margin-top:10px;"><?php esc_html_e( 'CONFIRM', 'event-tickets' );?></a>
					<?php else: ?>
						<button type="submit" name="tickets_process" value="1" class="tribe-events-button tribe-no-param" style="background:#00abef !important;margin-top:10px;"><?php esc_html_e( 'CONFIRM', 'event-tickets' );?></button>
					<?php endif; ?>
				</td>
			</tr>
		<?php endif; ?>
