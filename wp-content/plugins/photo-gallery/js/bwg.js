...
function addPricelist(pricelist) {
  tb_remove();
  jQuery("#image_pricelist_id").val(pricelist.id);
  spider_set_input_value('ajax_task', 'set_image_pricelist');
...
function spider_ajax_save(form_id, tr_group) {
  ...
  var gallery_source = (gallery_type_input == 'facebook') ? jQuery("#facebook_gallery_source").val() : jQuery("#gallery_source").val(); 
  var autogallery_image_number = (gallery_type_input == 'facebook') ? jQuery("#facebook_gallery_image_limit").val() : jQuery("#autogallery_image_number").val();
  var update_flag = (gallery_type_input == 'facebook') ? jQuery("input[name=facebook_update]:checked").val() : jQuery("input[name=update_flag]:checked").val(); 
  ...
  var gallery_type = gallery_type_input + (gallery_content_type == 1 ? "_post" : "");
  var gallery_type_old = jQuery("#gallery_type_old").val();
  ...
  var image_pricelist_id = jQuery("#image_pricelist_id").val();
  var remove_pricelist = jQuery("#remove_pricelist").val();
  ...
  post_data["gallery_type"] = gallery_type;
  post_data["gallery_type_old"] = gallery_type_old;
  post_data["gallery_source"] = gallery_source;
  post_data["autogallery_image_number"] = autogallery_image_number;
  post_data["update_flag"] = update_flag;
  ...
  post_data["image_pricelist_id"] = image_pricelist_id;
  post_data["remove_pricelist"] = remove_pricelist;
  ...
  jQuery.post(
    ...
    function (data) {
	  ...
	  if(ajax_task == "set_image_pricelist"){
		  jQuery("#image_pricelist_id").val("");
           for (var i in ids_array) {
            jQuery("#pricelist_id_"+i).val(image_pricelist_id);
          }
	  }
    }
    ...
    if (tr_count > bwg_save_count * tr_group) {
      ...
      if (ajax_task == 'ajax_apply') {
        var not_set_items = jQuery(data).find("#not_set_items").length>0 ?  jQuery(data).find("#not_set_items").val().split(",") : [];
        if (not_set_items.length > 0) {
          jQuery('#message_div').html("<strong><p>" + bwg_objectL10B.saved + " Selected pricelist item longest dimension greater than some original images dimensions.</p></strong>");
        }
        else {
      ...
      else if (ajax_task == 'set_image_pricelist') {
        var msg = jQuery(data).find(".bwg_msg").html();
        if (msg == undefined) {
          msg = "Pricelist Succesfully Set.";
        }
        jQuery('#draganddrop').attr('class', 'wd_updated');
        jQuery('#draganddrop').html("<strong><p>" + msg + "</p></strong>");
        jQuery('#draganddrop').show();
      }
      else if (ajax_task == "remove_image_pricelist") {
        jQuery("#remove_pricelist").val("");
        jQuery('#draganddrop').attr('class', 'wd_updated');
        jQuery('#draganddrop').html("<strong><p>Pricelist Succesfully Unset.</p></strong>");
        jQuery('#draganddrop').show();
      }
...
function bwg_check_checkboxes(bwg_alert) {
  if (typeof bwg_alert == undefined) {
    bwg_alert = false;
  }
  var flag = false;
  var ids_string = jQuery("#ids_string").val();
...
function bwg_change_theme_type(type) {
  ...
  jQuery("#type_Thumbnail").attr("style", "background-color: #00A0D2;border-bottom:1px solid #00A0D2;");
  jQuery("#type_Masonry").attr("style", "background-color: #00A0D2;border-bottom:1px solid #00A0D2;");
  jQuery("#type_Mosaic").attr("style", "background-color: #00A0D2;border-bottom:1px solid #00A0D2;");
  jQuery("#type_Slideshow").attr("style", "background-color: #00A0D2;border-bottom:1px solid #00A0D2;");
  jQuery("#type_Compact_album").attr("style", "background-color: #00A0D2;border-bottom:1px solid #00A0D2;");
  jQuery("#type_Extended_album").attr("style", "background-color: #00A0D2;border-bottom:1px solid #00A0D2;");
  jQuery("#type_Masonry_album").attr("style", "background-color: #00A0D2;border-bottom:1px solid #00A0D2;");
  jQuery("#type_Image_browser").attr("style", "background-color: #00A0D2;border-bottom:1px solid #00A0D2;");
  jQuery("#type_Blog_style").attr("style", "background-color: #00A0D2;border-bottom:1px solid #00A0D2;");
  jQuery("#type_Carousel").attr("style", "background-color: #00A0D2;border-bottom:1px solid #00A0D2;");
  jQuery("#type_Lightbox").attr("style", "background-color: #00A0D2;border-bottom:1px solid #00A0D2;");
  jQuery("#type_Navigation").attr("style", "background-color: #00A0D2;border-bottom:1px solid #00A0D2;");
  jQuery("#type_" + type).attr("style", "background-color: #fff; color: #00A0D2;position: relative;");
...
/*bulk embed handling*/
function bwg_bulk_embed(from, instagram_client_id){
  switch (from) {
    case 'instagram' : {
	  add_instagram_gallery(instagram_client_id, true);
	  break;
	}
	case 'facebook' : {
	  var mixed_gallery = true;
	  add_facebook_gallery(mixed_gallery);
	  break;
	}
  }
