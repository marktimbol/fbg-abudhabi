...
class BWGViewSlideshow {
  public function display($params, $from_shortcode = 0, $bwg = 0) {
    ...
    if (!$from) {
      ...
      if ($enable_slideshow_filmstrip) {
        ...
        if ($filmstrip_direction == 'horizontal') {
          ...
          $slideshow_filmstrip_width = (isset($params['slideshow_filmstrip_height']) ? esc_html($params['slideshow_filmstrip_height']) : '50');
      ...
      $enable_slideshow_filmstrip = $wd_bwg_options->slideshow_enable_filmstrip;
      if ($enable_slideshow_filmstrip) {
        $thumb_width = $wd_bwg_options->thumb_width;
        $thumb_height = $wd_bwg_options->thumb_height;
        if ($filmstrip_direction == 'horizontal') {
          ...
          $slideshow_filmstrip_width = (isset($params['filmstrip_height']) ? esc_html($params['filmstrip_height']) : '40');
          $thumb_ratio = $thumb_height / $thumb_width;
          $slideshow_filmstrip_height = round($thumb_ratio * $slideshow_filmstrip_width);
        }
      }
    ...
    $inline_style = $this->inline_styles($bwg, $theme_row, $params, $image_width, $image_height, $filmstrip_direction, $slideshow_filmstrip_height, $wd_bwg_options, $left_or_top, $width_or_height, $filmstrip_thumb_margin_hor, $slideshow_filmstrip_width, $image_rows, $watermark_position, $slideshow_title_position, $slideshow_description_position, $watermark_height, $watermark_width, $watermark_opacity, $watermark_font_size, $watermark_font, $watermark_color, $enable_slideshow_filmstrip);
              ...
              <div class="bwg_slideshow_filmstrip_<?php echo $bwg; ?>">
                <div class="bwg_slideshow_filmstrip_thumbnails_<?php echo $bwg; ?>">
                  ...
                  <div id="bwg_filmstrip_thumbnail_<?php echo $key; ?>_<?php echo $bwg; ?>" class="bwg_slideshow_filmstrip_thumbnail_<?php echo $bwg; ?> <?php echo (($image_row->id == $current_image_id) ? 'bwg_slideshow_thumb_active_' . $bwg : 'bwg_slideshow_thumb_deactive_' . $bwg); ?>">
                    <img style="width:<?php echo $image_thumb_width; ?>px; height:<?php echo $image_thumb_height; ?>px; margin-left: <?php echo $thumb_left; ?>px; margin-top: <?php echo $thumb_top; ?>px;" class="bwg_slideshow_filmstrip_thumbnail_img_<?php echo $bwg; ?>" src="<?php echo ( $is_embed ? "" : site_url() . '/' . $WD_BWG_UPLOAD_DIR) . $image_row->thumb_url; ?>" onclick="bwg_change_image_<?php echo $bwg; ?>(parseInt(jQuery('#bwg_current_image_key_<?php echo $bwg; ?>').val()), '<?php echo $key; ?>', data_<?php echo $bwg; ?>)" image_id="<?php echo $image_row->id; ?>" image_key="<?php echo $key; ?>" alt="<?php echo $image_row->alt; ?>"/>
                  </div>
                  <?php
                  }
                  ?>
