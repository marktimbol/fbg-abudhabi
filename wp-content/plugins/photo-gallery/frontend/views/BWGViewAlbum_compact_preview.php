...
class BWGViewAlbum_compact_preview {
  public function display($params, $from_shortcode = 0, $bwg = 0) {
    ...
    if ($from === "widget") {
      ...
      $params['popup_enable_ecommerce'] = $wd_bwg_options->popup_enable_ecommerce;
        ...
        <form id="gal_front_form_<?php echo $bwg; ?>" method="post" action="#" data-current="<?php echo $bwg; ?>">
          ...
          <div id="<?php echo $form_child_div_id; ?>" style="<?php echo $form_child_div_style; ?>">
                ...
                foreach ($image_rows as $image_row) {
                  ...
                  if ($album_view_type == 'thumbnail') {
                    ...
                    <span class="bwg_standart_thumb_<?php echo $bwg; ?>">
                      ...
                      ?>
                    <?php
                      if (defined('WDPG_ECOMMERCE_NAME') && is_plugin_active(WDPG_ECOMMERCE_NAME.'/'.'photo-gallery-ecommerce.php') && $params['ecommerce_icon'] == 'show' && $image_row->pricelist_id) {
                        ?>
                        <span class="bwg_ecommerce_spun1_<?php echo $bwg; ?>">
                            <span class="bwg_ecommerce_spun2_<?php echo $bwg; ?>">
                              <i title="<?php echo __('Open', 'bwg'); ?>" class="bwg_ctrl_btn bwg_open fa fa-share-square" ></i>
                  ...
                  <span class="bwg_masonry_thumb_spun_<?php echo $bwg; ?>">
                    <a <?php echo ($params['thumb_click_action'] == 'open_lightbox' ? (' class="bwg_lightbox_' . $bwg . '"' . ($wd_bwg_options->enable_seo ? ' href="' . ($is_embed ? $image_row->thumb_url : site_url() . '/' . $WD_BWG_UPLOAD_DIR . $image_row->image_url) . '"' : '') . ' data-image-id="' . $image_row->id . '" data-gallery-id="' . $album_gallery_id . '"') : ($params['thumb_click_action'] == 'redirect_to_url' && $image_row->redirect_url ? 'href="' . $image_row->redirect_url . '" target="' .  ($params['thumb_link_target'] ? '_blank' : '')  . '"' : '')) ?>>
                      <img class="bwg_masonry_thumb_<?php echo $bwg; ?> bwg_img_clear bwg_img_custom" id="<?php echo $image_row->id; ?>" src="<?php echo ($is_embed ? "" : site_url() . '/' . $WD_BWG_UPLOAD_DIR) . $image_row->thumb_url; ?>" alt="<?php echo $image_row->alt; ?>" />
                    ...
                    if ($params['compuct_album_image_title'] == 'hover' || $params['compuct_album_image_title'] == 'show') {
                    ?>
            ...
            if ($album_view_type == 'mosaic' && $type == 'gallery') {
              if ($params['compuct_album_mosaic_hor_ver'] == 'vertical') {
                ?>
                /* Vertical.*/
                function bwg_mosaic_<?php echo $bwg; ?>(event_type) {
                  <?php
                  if ($params['compuct_album_resizable_mosaic'] == 1) {
                    ?>
                    if (jQuery(window).width() >= 1920) {
                      var thumbnail_width = (1 + jQuery(window).width() / 1920) * <?php echo $params['compuct_album_image_thumb_width']; ?>;
                    }
                    else if (jQuery(window).width() <= 640) {
                      var thumbnail_width = jQuery(window).width()/640*<?php echo $params['compuct_album_image_thumb_width']; ?>;
                    }
                    else {
                      var thumbnail_width = <?php echo $params['compuct_album_image_thumb_width']; ?>;
                    }
                    <?php
                  }
                  else {
                  ...
                  /* Resize.*/
                  var divwidth = jQuery("#bwg_mosaic_thumbnails_div_<?php echo $bwg; ?>").width()/100 * <?php echo $params['compuct_album_mosaic_total_width'] ?>;
                ...
                function bwg_mosaic_<?php echo $bwg; ?>(event_type) {
                  ...
                  if ($params['compuct_album_resizable_mosaic'] == 1) {
                    ?>
                    if (jQuery(window).width() >= 1920) {
                      var thumbnail_height = (1 + jQuery(window).width() / 1920) * <?php echo $params['compuct_album_image_thumb_height']; ?>;
                    }
                    else if (jQuery(window).width() <= 640) {
                      var thumbnail_height = jQuery(window).width() / 640 * <?php echo $params['compuct_album_image_thumb_height']; ?>;
                    }
                    else {
                      var thumbnail_height = <?php echo $params['compuct_album_image_thumb_height']; ?>;
                    }
                    <?php
                  }
                  else {
              ...
              function show_title_always_show_<?php echo $bwg; ?>() {
                ...
                ?>
                /* If not $params['compuct_album_image_title'] == 'show', function body is empty.*/
              }
