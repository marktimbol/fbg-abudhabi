...
class BWGControllerGalleries_bwg {
  ...
  public function save() {
    $msg = __("Item Succesfully Saved.","bwg_back");
    if(defined('WDPG_ECOMMERCE_NAME') && is_plugin_active(WDPG_ECOMMERCE_NAME.'/'.'photo-gallery-ecommerce.php')){
        ...
        if(empty($not_set_items) === false){
            $msg .= __(" Selected pricelist item longest dimension greater than some original images dimensions.","bwg_back");
        }
    }
  ...
  public function set_image_pricelist(){
    global $wpdb;
    global $WD_BWG_UPLOAD_DIR;
	$gallery_id = ((isset($_POST['current_id'])) ? esc_html(stripslashes($_POST['current_id'])) : 0);
	
    $image_ids_col = $wpdb->get_col($wpdb->prepare('SELECT id FROM ' . $wpdb->prefix . 'bwg_image WHERE gallery_id="%d"', $gallery_id));
	$pricelist_id = $_POST["image_pricelist_id"];
    ...
    foreach ($image_ids_col as $image_id) {
      if (isset($_POST['check_' . $image_id]) || isset($_POST['check_all_items'])) {
		...
		if($pricelist_id){
		  $file_path = str_replace("thumb",".original", htmlspecialchars_decode( ABSPATH . $WD_BWG_UPLOAD_DIR .$_POST['thumb_url_' . $image_id], ENT_COMPAT | ENT_QUOTES));
	...
	if(empty( $not_set_items) === false){
	  echo "<div class='bwg_msg'>".__('Selected pricelist item longest dimension greater than some original images dimensions.', 'bwg_back') ."</div>";  
	}
    ...
    if($gallery_id){
       ...
       foreach($rows as $row){
          if($row->item_longest_dimension){  
              $file_path = str_replace("thumb",".original", htmlspecialchars_decode( ABSPATH . $WD_BWG_UPLOAD_DIR .$row->thumb_url, ENT_COMPAT | ENT_QUOTES));
