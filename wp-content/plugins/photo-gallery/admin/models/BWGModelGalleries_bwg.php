...
class BWGModelGalleries_bwg {
  ...
  public function get_image_rows_data($gallery_id) {
	...
	if(defined('WDPG_ECOMMERCE_NAME') && is_plugin_active(WDPG_ECOMMERCE_NAME.'/'.'photo-gallery-ecommerce.php')){	
	  ...
	  $row = $wpdb->get_results("SELECT T_IMAGE.*, T_PRICELISTS.title AS priselist_name, T_PRICELIST_ITEMS.item_longest_dimension, T_PRICELISTS.sections FROM " .$wpdb->prefix . "bwg_image AS T_IMAGE  LEFT JOIN ". $wpdb->prefix ."wdpg_ecommerce_pricelists AS T_PRICELISTS ON T_IMAGE.pricelist_id = T_PRICELISTS.id LEFT JOIN ( SELECT  MAX(item_longest_dimension) AS item_longest_dimension, pricelist_id  FROM  " . $wpdb->prefix . "wdpg_ecommerce_pricelist_items GROUP BY pricelist_id) AS T_PRICELIST_ITEMS ON T_PRICELIST_ITEMS.pricelist_id = T_PRICELISTS.id  " . $where . " AND gallery_id='" . $gallery_id . "' " . $image_order_by . " LIMIT " . $limit . ",".$this->per_page);
      
      foreach($row as $value){
          $value->not_set_items = 0;
          if($value->item_longest_dimension && strpos($value->sections,"downloads")!== false){  
              $file_path = str_replace("thumb",".original", htmlspecialchars_decode( ABSPATH . $WD_BWG_UPLOAD_DIR .$value->thumb_url, ENT_COMPAT | ENT_QUOTES));
