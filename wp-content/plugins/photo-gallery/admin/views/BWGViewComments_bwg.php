...
class BWGViewComments_bwg {
  ...
  public function display() {
    ...
    $pager = 0;
    $comments_button_array = array(
      'publish_all' => __('Publish', 'bwg_back'),
      'unpublish_all' => __('Unpublish', 'bwg_back'),
      'delete_all' => __('Delete', 'bwg_back')
    );
    ?>
    <form class="wrap bwg_form" id="comments_form" method="post" action="admin.php?page=comments_bwg" style="width: 98%; float: left;">
    <?php wp_nonce_field( 'comments_bwg', 'bwg_nonce' ); ?>
      <div>
        <span class="comment_icon"></span>
        <h2>
          <?php echo __('Comments', 'bwg_back'); ?>
        </h2>
      </div>
      <?php WDWLibrary::search(__('Name','bwg_back'), $search_value, 'comments_form',''); ?>
      ...
      <div style="float:left; margin-right:10px;">
        ...
        <select class="select_icon bulk_action" style="width:150px; margin-right:5px;">
          ...
          <?php 
          foreach ($comments_button_array as $key => $value) {
            ?>
        ...
        <input class="wd-btn wd-btn-primary wd-btn-icon wd-btn-apply" type="button" title="<?php _e("Apply", "bwg_back"); ?>" onclick="if (!bwg_bulk_actions('.bulk_action', 'comments_page')) {return false;}" value="<?php _e("Apply", "bwg_back"); ?>" />       
        </div>
        <?php
        WDWLibrary::search_select('', 'search_select_gal_value', $search_select_gal_value, $this->model->get_galleries(), 'comments_form');
        WDWLibrary::search_select('', 'search_select_value', $search_select_value, $this->model->get_images($search_select_gal_value), 'comments_form');
        WDWLibrary::html_page_nav($page_nav['total'], $pager++, $page_nav['limit'], 'comments_form', $per_page,'position_pag');
        ?>
      </div>
      <table class="wp-list-table widefat fixed pages">
        <thead>
          ...
          <th class="sortable <?php if ($order_by == 'comment') {echo $order_class;} ?>">
            <a onclick="spider_set_input_value('task', '');
                        ...
                        spider_form_submit(event, 'comments_form')" href="">
              <span><?php echo __('Comment', 'bwg_back'); ?></span><span class="sorting-indicator"></span>
            </a>
          </th>
          <?php
          ...
          <th class="<?php if ($order_by == 'mail') {echo $order_class;} ?>">
            <a onclick="spider_set_input_value('task', '');
                        ...
                        spider_form_submit(event, 'comments_form')" href="">
              <span><?php echo __('Email', 'bwg_back'); ?></span><span class="sorting-indicator"></span>
            </a>
          </th>
          <?php
          }
          ...
          if ($rows_data) {
            foreach ($rows_data as $row_data) {
              ...
              <tr id="tr_<?php echo $row_data->id; ?>" <?php echo $alternate; ?>>
                ...
                ?>
                <td class="table_big_col publish_icon"><a style="background-image:url('<?php echo WD_BWG_URL . '/images/icons/' . $published_image . '.png'; ?>'); background-repeat: no-repeat; display: inline-block; width: 18px; height: 22px;margin: 3px; vertical-align: middle;background-size: contain;" onclick="spider_set_input_value('task', '<?php echo $published; ?>');spider_set_input_value('current_id', '<?php echo $row_data->id; ?>');spider_form_submit(event, 'comments_form')" href=""></a></td>
                ...
                spider_form_submit(event, 'comments_form')}
                else {
                  return false;
                }" href=""></a></td>
              </tr>
