...
class BWGViewGalleries_bwg {
      ...
      <div>
        ...
        <h2>
          ...
          <a href="" id="galleries_id"  class="add-new-h2" onclick="spider_set_input_value('task', 'add');
                                                 spider_form_submit(event, 'galleries_form')"><?php echo __('Add new', 'bwg_back'); ?></a>
        </h2>
      </div>
          ...
          if ($rows_data) {
            foreach ($rows_data as $row_data) {
              ...
              <tr id="tr_<?php echo $row_data->id; ?>" <?php echo $alternate; ?>>
                ...
                <td class="connectedSortable handles table_small_col"><div title="Drag to re-order" class="handle" style="margin:3px auto 0 auto; vertical-align"></div></td>
  ...
  public function edit($id) {
    ...
    <script>
      ...
      function bwg_add_image(files) {
        ...
        for (var i in files) {
          ...
          $query_url = wp_nonce_url( admin_url('admin-ajax.php'), 'editThumb', 'bwg_nonce' );
          ?>
          a_thumb.setAttribute('href', "<?php echo add_query_arg(array('action' => 'editThumb', 'type' => 'display'/*thumb_display*/, 'width' => '650', 'height' => '500'), $query_url); ?>&FACEBOOK_POST="+is_facebook_post+"&fb_post_url="+fb_post_url+"&image_id=" + bwg_j + "&TB_iframe=1");
          ...
          $query_url = wp_nonce_url( admin_url('admin-ajax.php'), 'editThumb', 'bwg_nonce' );
          ?>
          a_filename.setAttribute('href', "<?php echo add_query_arg(array('action' => 'editThumb', 'type' => 'display', 'width' => '800', 'height' => '500'), $query_url); ?>&FACEBOOK_POST="+is_facebook_post+"&fb_post_url="+fb_post_url+"&image_id=" + bwg_j + "&TB_iframe=1");
    ...
    <script language="javascript" type="text/javascript" src="<?php echo WD_BWG_URL . '/js/bwg_embed.js?ver='; ?><?php echo wd_bwg_version(); ?>"></script>
    <form class="wrap bwg_form" method="post" id="galleries_form" action="admin.php?page=galleries_bwg" style="width: 98%; float: left;">
      <?php wp_nonce_field( 'galleries_bwg', 'bwg_nonce' ); ?>
      <div>
        <span class="gallery-icon"></span>
      ...
      <div style="float:right;">
        <input id="save_gall" class="wd-btn wd-btn-primary wd-btn-icon wd-btn-save" type="button" onclick="if (spider_check_required('name', 'Name') || bwg_check_instagram_gallery_input('<?php echo $wd_bwg_options->instagram_access_token ?>') ) {return false;};
                                                     spider_set_input_value('page_number', '1');
                                                     spider_set_input_value('ajax_task', 'ajax_save');
                                                     spider_ajax_save('galleries_form');
                                                     spider_set_input_value('task', 'save');" value="<?php echo __('Save', 'bwg_back'); ?>" />
        <input class="wd-btn wd-btn-primary wd-btn-icon wd-btn-apply" type="button" onclick="if (spider_check_required('name', 'Name') || bwg_check_instagram_gallery_input('<?php echo $wd_bwg_options->instagram_access_token ?>') ) {return false;};
                                                     spider_set_input_value('ajax_task', 'ajax_apply');
                                                     spider_ajax_save('galleries_form');" value="<?php echo __('Apply', 'bwg_back'); ?>" />
        <input class="wd-btn wd-btn-primary wd-btn-icon wd-btn-cancel" type="submit" onclick="spider_set_input_value('page_number', '1');
                                                     spider_set_input_value('task', 'cancel')" value="<?php echo __('Cancel', 'bwg_back'); ?>" />
      </div>
      <table style="clear:both; width:99.9%">
        <tbody>
          <tr>
            <td class="spider_label_galleries"><label for="name"><?php echo __('Name:', 'bwg_back'); ?> <span style="color:#FF0000;">*</span> </label></td>
            <td><input type="text" id="name" name="name" value="<?php echo $row->name; ?>" size="39" class="bwg_requried"/></td>
          </tr>
          <tr>
            <td class="spider_label_galleries"><label for="slug"><?php echo __('Slug:', 'bwg_back'); ?> </label></td>
            <td><input type="text" id="slug" name="slug" value="<?php echo $row->slug; ?>" size="39" /></td>
          </tr>
          <tr>
            <td class="spider_label_galleries"><label for="gallery_type"><?php echo __('Gallery content type:', 'bwg_back'); ?> </label></td>
            <td>
              <select name="gallery_type" id="gallery_type" class="select_icon" style="width:150px;" onchange="bwg_gallery_type('<?php echo $wd_bwg_options->instagram_access_token ?>');">
                <option value="" <?php if ($gallery_type == '') echo 'selected="selected"'; ?>><?php echo __('Mixed', 'bwg_back'); ?></option>
                <option value="instagram" <?php if ($gallery_type == 'instagram') echo 'selected="selected"'; ?>><?php echo __('Instagram only', 'bwg_back'); ?></option> 
                <?php if($wd_bwg_fb) : ?>
                  <option value="facebook" <?php if ($gallery_type == 'facebook') echo 'selected="selected"'; ?>><?php echo __('Facebook', 'bwg_back'); ?></option>				
                <?php endif; ?>
              </select>
		 ...
		 <!-- instagram gallery -->
        <tbody id="add_instagram_gallery"><!-- /*use this in javascript*/-->
          <tr id='tr_gallery_source' style='display:<?php echo ($gallery_type == 'instagram' ? 'table-row': 'none'); ?>;'>
            <td class="spider_label_galleries"><label for="gallery_source"><?php echo __('Instagram username:', 'bwg_back'); ?> </label></td>
            <td><input type="text" id="gallery_source" name="gallery_source" value="<?php echo $gallery_source; ?>" size="64" class="bwg_requried"/></td>

          </tr>
          <tr id='tr_autogallery_image_number' style='display:<?php echo ($gallery_type == 'instagram' ? 'table-row': 'none'); ?>;'>
            <td class="spider_label_galleries"><label for="autogallery_image_number"><?php echo __('Number of Instagram recent posts to add to gallery:', 'bwg_back'); ?> </label></td>
            <td><input type="number" id="autogallery_image_number" name="autogallery_image_number" value="<?php echo $autogallery_image_number; ?>" /></td>
          </tr>
          <tr id='tr_instagram_post_gallery' style='display:<?php echo ($gallery_type == 'instagram' ? 'table-row': 'none'); ?>;'>
            <td class="spider_label_galleries"><label><?php echo __('Instagram embed type: ', 'bwg_back'); ?></label></td>
            <td>
                <input type="radio" class="inputbox" id="instagram_post_gallery_0" name="instagram_post_gallery" <?php echo ($instagram_post_gallery ? '' : 'checked="checked"'); ?> value="0" >
                <label for="instagram_post_gallery_0"><?php echo __('Content', 'bwg_back'); ?></label>&nbsp;
                <input type="radio" class="inputbox" id="instagram_post_gallery_1" name="instagram_post_gallery" <?php echo ($instagram_post_gallery ? 'checked="checked"' : ''); ?> value="1" >
                <label for="instagram_post_gallery_1"><?php echo __('Whole post', 'bwg_back'); ?></label>
              </td>
          </tr>
          <tr id='tr_update_flag' style='display:<?php echo ($gallery_type == 'instagram' ? 'table-row': 'none'); ?>;'>
            <td class="spider_label_galleries"><label><?php echo __('Gallery autoupdate option:', 'bwg_back'); ?> </label></td>
            <td>
                <input type="radio" class="inputbox" id="update_flag_0" name="update_flag" <?php echo ($update_flag =='' ? 'checked="checked"':''); ?> value="" >
                <label for="update_flag_0"><?php echo __('No update', 'bwg_back'); ?></label>&nbsp;
                <input type="radio" class="inputbox" id="update_flag_1" name="update_flag" <?php echo ($update_flag =='add' ? 'checked="checked"':''); ?> value="add" >
                <label for="update_flag_1"><?php echo __('Add new media, keep old ones published.', 'bwg_back'); ?></label>&nbsp;
                <input type="radio" class="inputbox" id="update_flag_2" name="update_flag" <?php echo ($update_flag =='replace' ? 'checked="checked"':''); ?> value="replace" >
                <label for="update_flag_2"><?php echo __('Add new media, unpublish old ones.', 'bwg_back'); ?></label>&nbsp;
              </td>
          </tr>
          <tr id='tr_instagram_gallery_add_button' style='display:<?php echo (($gallery_type == 'instagram' && $images_count == 0) ? 'table-row': 'none'); ?>;'>
            <td class="spider_label_galleries"></td>
            <td>
              <input id="instagram_gallery_add_button" class="button-primary" type="button" onclick="add_instagram_gallery('<?php echo $wd_bwg_options->instagram_access_token ?>');" value="Add Instagram Gallery" />    
            </td>
          </tr>
        </tbody>
        <?php if($wd_bwg_fb): ?>
          <!-- facebook gallery -->		
          <tbody id="add_facebook_gallery" <?php echo ($gallery_type == 'facebook' ? '': 'style="display:none"'); ?> >
            <tr id='tr_facebook_gallery_source' >
              <td class="spider_label_galleries"><label for="facebook_gallery_source"><?php echo __('Facebook album url: ', 'bwg_back'); ?></label></td>
              <td><input type="text" id="facebook_gallery_source" name="facebook_gallery_source" value="<?php echo $gallery_source; ?>" size="64" /></td>
            </tr>
            <tr id='tr_facebook_album_limit'>
              <td class="spider_label_galleries"><label for="facebook_gallery_image_limit"><?php echo __('Images limit: ', 'bwg_back'); ?></label></td>
              <td><input type="number" id="facebook_gallery_image_limit" name="facebook_gallery_image_limit" value="<?php echo $autogallery_image_number; ?>" /></td>
            </tr>
            <!-- <tr id='tr_facebook_content_type'>
              <td class="spider_label_galleries"><label>Content type: </label></td>
              <td>
                  <input type="radio" class="inputbox" id="facebook_content_type_0" name="facebook_content_type" <?php echo ($facebook_post_gallery ? '' : 'checked="checked"'); ?> value="0" >
                  <label for="facebook_content_type_0">Regular</label>&nbsp;
                  <input type="radio" class="inputbox" id="facebook_content_type_1" name="facebook_content_type" <?php echo ($facebook_post_gallery ? 'checked="checked"' : ''); ?> value="1" >
                  <label for="facebook_content_type_1">Post</label>
                </td>
            </tr> -->
            <tr id='tr_facebook_update_flag'>
              <td class="spider_label_galleries"><label><?php echo __('Gallery autoupdate option: ', 'bwg_back'); ?></label></td>
              <td>
                  <input type="radio" class="inputbox" id="facebook_update_0" name="facebook_update" <?php echo ($update_flag =='' ? 'checked="checked"':''); ?> value="" >
                  <label for="facebook_update_0"><?php echo __('No', 'bwg_back'); ?></label>&nbsp;
                  <input type="radio" class="inputbox" id="facebook_update_2" name="facebook_update" <?php echo ($update_flag =='replace' ? 'checked="checked"':''); ?> value="replace" >
                  <label for="facebook_update_2"><?php echo __('Yes', 'bwg_back'); ?></label>&nbsp;
                </td>
            </tr>
            <tr id='tr_facebook_gallery_add_button'>
              <td class="spider_label_galleries"></td>
              <td>
                <input id="facebook_gallery_add_button" class="button-primary" type="button" onclick="add_facebook_gallery(false)" value="Add facebook Gallery" />    
              </td>
            </tr>
          </tbody>
        <?php endif; ?>
        <tbody>      
          <tr>
            <td class="spider_label_galleries"><label for="description"><?php echo __('Description: ', 'bwg_back'); ?></label></td>
            <td>
          ...
          <tr>
            <td class="spider_label_galleries"><label><?php echo __('Author:', 'bwg_back'); ?> </label></td>
            <td><?php echo get_userdata($row->author)->display_name; ?></td>
          </tr>
          <tr>
            <td class="spider_label_galleries"><label><?php echo __('Published: ', 'bwg_back'); ?></label></td>
            <td>
              <input type="radio" class="inputbox" id="published0" name="published" <?php echo (($row->published) ? '' : 'checked="checked"'); ?> value="0" >
              <label for="published0"><?php echo __('No', 'bwg_back'); ?></label>
              <input type="radio" class="inputbox" id="published1" name="published" <?php echo (($row->published) ? 'checked="checked"' : ''); ?> value="1" >
              <label for="published1"><?php echo __('Yes', 'bwg_back'); ?></label>
            </td>
          </tr>
          <tr>
            <td class="spider_label_galleries"><label for="url"><?php echo __('Preview image:', 'bwg_back'); ?></label></td>
            <td>
              <?php 
  ...
  public function image_display($id) {
    ...
    <div class="buttons_div_left">
      <?php
      $query_url = add_query_arg(array('action' => 'addImages', 'width' => '700', 'height' => '550', 'extensions' => 'jpg,jpeg,png,gif', 'callback' => 'bwg_add_image', 'show_intro' => '1'), admin_url('admin-ajax.php'));
      $query_url = wp_nonce_url($query_url, 'addImages', 'bwg_nonce');
      $query_url = add_query_arg(array('TB_iframe' => '1'), $query_url);
      ?>
      ...
      <input id="show_add_embed" class="wd-btn wd-btn-primary wd-btn-icon wd-btn-media" title="<?php _e('Embed Media', 'bwg_back'); ?>" style="<?php if($gallery_type !='') {echo 'display:none';} ?>" type="button" onclick="jQuery('.opacity_add_embed').show(); jQuery('#add_embed_help').hide(); return false;" value="<?php _e('Embed Media', 'bwg_back'); ?>" />
      <input id="show_bulk_embed" class="wd-btn wd-btn-primary wd-btn-icon wd-btn-bulk_media" title="<?php _e('Social Bulk Embed', 'bwg_back'); ?>" style="<?php if($gallery_type !='') {echo 'display:none';} ?>" type="button" onclick="jQuery('.opacity_bulk_embed').show(); return false;" value="<?php _e('Social Bulk Embed', 'bwg_back'); ?>" />
    ...
    <?php 
		if (defined('WDPG_ECOMMERCE_NAME') && is_plugin_active(WDPG_ECOMMERCE_NAME.'/'.'photo-gallery-ecommerce.php')) {
			...
			$query_url = add_query_arg(array('action' => 'add_pricelist', 'page' => 'pricelists_pge', 'task' => 'explore', 'width' => '650', 'height' => '500', 'TB_iframe' => '1' ), $query_url);
      ?>
    <div class="buttons_div_left" style="clear:both;">	
      <div><?php _e('Pricelist', 'bwg_back'); ?></div>
      <a  onclick="return bwg_check_checkboxes(1);" href="<?php echo $query_url;?>" title="<?php _e('Add pricelist', 'bwg_back'); ?>" class="wd-btn wd-btn-primary wd-btn-icon wd-btn-add thickbox thickbox-preview" ><?php _e('Add pricelist', 'bwg_back'); ?></a>
      <input type="button" value="<?php _e('Remove pricelist', 'bwg_back'); ?>" title="<?php _e('Remove pricelist','bwg_back'); ?>" class="wd-btn wd-btn-primary-red wd-not-image"  onclick="if(!bwg_check_checkboxes(1) ){ return false;}
                                                   else if(confirm('Do you want to remove pricelist from selected items?') ) {
                                                     spider_set_input_value('ajax_task', 'remove_pricelist_all');
                                                     spider_ajax_save('galleries_form');
                                                     return false;
                                                   } else {
                                                     return false;
    ...
    <div class="opacity_resize_image opacity_add_embed opacity_image_desc opacity_bulk_embed bwg_opacity_media" onclick="jQuery('.opacity_add_embed').hide(); jQuery('.opacity_bulk_embed').hide(); jQuery('.opacity_resize_image').hide(); jQuery('.opacity_image_desc').hide();"></div>       
      <div id="add_embed" class="opacity_add_embed bwg_add_embed">
        ...
        <div class="spider_description">
          <p><?php _e('Enter YouTube, Vimeo, Instagram, Facebook, Flickr or Dailymotion URL here.', 'bwg_back'); ?> <a onclick="jQuery('#add_embed_help').show();" style='text-decoration: underline; color: blue; cursor: pointer;'><?php _e('Help', 'bwg_back'); ?></a></p>
        </div>
        <div id="add_embed_help" class= "opacity_add_embed bwg_add_embed" style="display: none;">
          ...
          <p><b>Facebook</b> </br>         
            <?php echo __('Photo URL example:', 'bwg_back'); ?> <i>https://www.facebook.com/WebDorado/photos/pb.436551809728904.-2207520000.1442409849./<i style="text-decoration:underline;">1007024672681612</i></i>.</br>
            <?php _e('Video URL example:', 'bwg_back'); ?> <i style="">https://www.facebook.com/WebDorado/videos/vb.436551809728904/<i style="text-decoration:underline;">1013555838695162</i></i>.</br>
            ...
            <?php _e('Note that media must have public privacy.', 'bwg_back'); ?>
          </p>	
      ...
      <div id="bulk_embed" class="opacity_bulk_embed bwg_bulk_embed">
        ...
        <input class="wd-btn wd-btn-primary wd-btn-icon wd-btn-add" type="button" onclick="bwg_bulk_embed('instagram', '<?php echo $wd_bwg_options->instagram_access_token ?>');" value="<?php _e('Add to gallery', 'bwg_back'); ?>" style="float: right; margin-left: 5px;" />
        <div class="spider_description"></div>
    		<table>
    	    <?php if ($wd_bwg_fb): ?>
            <thead>
              <tr>
                <td class="spider_label_galleries"><label for="bulk_embed_from"><?php echo __('Bulk embed from:', 'bwg_back'); ?> </label></td>
                <td>
                  <input type="radio" class="inputbox" id="bulk_embed_from_instagram" name="bulk_embed_from" onclick="jQuery('#facebook_bulk_params').hide();jQuery('#instagram_bulk_params').show();jQuery('#bulk_embed').find('.wd-btn-add').attr('onclick', 'bwg_bulk_embed(\'instagram\', \'<?php echo $wd_bwg_options->instagram_access_token ?>\')')" checked="checked" value="instagram" >
                  <label for="bulk_embed_from_instagram">Instagram</label>&nbsp; 
                  <input type="radio" class="inputbox" id="bulk_embed_from_facebook" name="bulk_embed_from" onclick="jQuery('#instagram_bulk_params').hide();jQuery('#facebook_bulk_params').show();jQuery('#bulk_embed').find('.wd-btn-add').attr('onclick', 'bwg_bulk_embed(\'facebook\', \'\')')"  value="facebook" >
          ...
          <tbody id="instagram_bulk_params">
            <tr id='popup_tr_instagram_gallery_source' style='display:table-row'>
              <td class="spider_label_galleries"><label for="popup_instagram_gallery_source">Instagram <?php echo __('username:', 'bwg_back'); ?> </label></td>
              <td><input type="text" id="popup_instagram_gallery_source" name="popup_instagram_gallery_source" value="" size="64" class="bwg_requried"/></td>
            </tr>
            <tr id='popup_tr_instagram_image_number' style='display:table-row'>
              <td class="spider_label_galleries"><label for="popup_instagram_image_number"><?php echo __('Number of Instagram recent posts to add to gallery:', 'bwg_back'); ?> </label></td>
            ...
            <tr id='popup_tr_instagram_post_gallery' style='display:table-row'>
              <td class="spider_label_galleries"><label>Instagram <?php echo __('embed type:', 'bwg_back'); ?> </label></td>
              <td>
                ...
                <label for="popup_instagram_post_gallery_0"><?php echo __('Content', 'bwg_back'); ?></label>&nbsp;
                <input type="radio" class="inputbox" id="popup_instagram_post_gallery_1" name="popup_instagram_post_gallery" value="1" >
                <label for="popup_instagram_post_gallery_1"><?php echo __('Whole post', 'bwg_back'); ?></label>
              </td>
            </tr>
    		  </tbody>
          <?php if ($wd_bwg_fb): ?>
            ...
            <tfoot id="facebook_bulk_params" style="display:none">
              <tr id='popup_tr_facebook_gallery_album_url' style='display:table-row'>
                <td class="spider_label_galleries"><label for="popup_facebook_gallery_album_url">Facebook <?php echo __('album url:', 'bwg_back'); ?> </label></td>
              ...
              <tr id='popup_tr_facebook_gallery_album_limit' style='display:table-row'>
                <td class="spider_label_galleries"><label for="popup_facebook_gallery_album_limit"><?php echo __('Images limit:', 'bwg_back'); ?> </label></td>
         ...
         <th class="check-column table_small_col table_medium_col manage-column column-title sorted asc" style="margin: 0px auto 0px 15px; width: 60px; vertical-align: middle;">
          ...
          <th class="sortable table_extra_large_col <?php if ($image_order_by == 'alt') {echo $order_class;} ?>">
            <a onclick="spider_set_input_value('task', '');
                        ...
                        spider_ajax_save('galleries_form');">
              <span><?php echo __('Alt/Title', 'bwg_back'); ?><?php if ($wd_bwg_options->thumb_click_action != 'open_lightbox') { ?><br /><?php echo __('Redirect', 'bwg_back'); ?> URL<?php } ?></span><span class="sorting-indicator"></span>
            </a>
          </th>
          ...
          if ($rows_data) {
            foreach ($rows_data as $row_data) {
              ...
              <tr id="tr_<?php echo $row_data->id; ?>" <?php echo $alternate; ?>>
                ...
                $query_url = add_query_arg(array('action' => 'editThumb', 'type' => 'display'/*thumb_display*/, 'image_id' => $row_data->id, 'width' => '800', 'height' => '500', 'instagram_post_width' => $instagram_post_width, 'instagram_post_height' => $instagram_post_height, 'FACEBOOK_POST' => $is_facebook_post, 'fb_post_url' => $fb_post_url), admin_url('admin-ajax.php'));
                ...
                <td class="table_extra_large_col">
                  ...
                  if (defined('WDPG_ECOMMERCE_NAME') && is_plugin_active(WDPG_ECOMMERCE_NAME . '/photo-gallery-ecommerce.php')) {
                    ...
                    $unset = $priselist_name == "Not for sale" ? "" : " <span onclick='bwg_remove_pricelist(this);' data-image-id= '".$row_data->id."' data-pricelist-id='".$row_data->pricelist_id."' class ='spider_delete_img_small' style='margin-top: -2px;margin-left: 3px;'></span>";
                    echo "<div><strong>".$priselist_name." </strong>".$unset."</div>";
                    $not_set_text = $row_data->not_set_items == 1 ? __('Selected pricelist item longest dimension greater than some original images dimensions.', 'bwg_back') : "";
                ...
                <td class="table_big_col publish_icon"><a style="background-image:url('<?php echo WD_BWG_URL . '/images/icons/' . $published_image . '.png'; ?>'); background-repeat: no-repeat; display: inline-block; width: 18px; height: 22px;margin: 3px; vertical-align: middle;background-size: contain;" title="<?php echo $unpublish; ?>" title="<?php echo $unpublish; ?>" onclick="spider_set_input_value('ajax_task', 'image_<?php echo $published; ?>');
                                                      spider_set_input_value('image_current_id', '<?php echo $row_data->id; ?>');
                                                      spider_ajax_save('galleries_form');"></a></td>
                <td class="table_big_col spider_delete_button" style="<?php if($gallery_type != '' && $update_flag != '') {echo 'display:none';} ?>"><a class="bwg_img_remove" title='<?php echo __('Delete', 'bwg_back'); ?>' onclick="if (confirm('<?php echo addslashes(__('Do you want to delete selected items?', 'bwg_back')); ?>')) {
