...
class Pro_Ad_Stats_Year_List_Table extends WP_List_Table {
	...
	{	
        ...
        switch($column_name){
            ...
            default:
				return '<a href="javascript:void(0)" class="time_frame_btn" s_type="month" year="'.date_i18n('Y', $item['time']).'" month="'.date_i18n('n', $item['time']).'" day="'.date_i18n('j', $item['time']).'" select="'.$item['select'].'">'.date_i18n('F', $item['time']).'</a>';
	...
	*/
    ...
    function prepare_items( $args = array() ) {
		...
		for( $i = 1; $i <= 12; $i++ )
		{
			$cnt = $wppas_stats->get_monthly_stats(
				array(
					'stats'   => $args['stats'],
					'year'    => $args['year'],
					'month'   => $i,
			...
			$impressions = $wppas_stats->get_monthly_stats(
				array(
					'stats'   => $args['stats'],
					'year'    => $args['year'],
					'month'   => $i,
			...
			$clicks = $wppas_stats->get_monthly_stats(
				array(
					'stats'   => $args['stats'],
					'year'    => $args['year'],
					'month'   => $i,
			...
			$am_days = cal_days_in_month(CAL_GREGORIAN, $i, $args['year']);
			for( $d = 1; $d <= $am_days; $d++ )
			{
				if( !empty($args['stats']) && !empty($args['stats'][$args['year']][$i][$d]) && $cnt)
				{
					foreach($args['stats'][$args['year']][$i][$d] as $today_statss)
					{
						foreach( $today_statss as $today_stats)
						{
							...
							if($match)
							{
								$data[$i]['time'] = mktime(0,0,0,$i,1, $args['year']);
								$data[$i]['hits'] = $cnt;
