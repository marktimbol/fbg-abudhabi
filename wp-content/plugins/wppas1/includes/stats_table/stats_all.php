...
class Pro_Ad_All_Stats_List_Table extends WP_List_Table {
	...
	{	
        ...
        switch($column_name){
            ...
            default:
				return '<a href="javascript:void(0)" class="time_frame_btn" s_type="year" year="'.date_i18n('Y', $item['time']).'" month="'.date_i18n('n', $item['time']).'" day="'.date_i18n('j', $item['time']).'" select="'.$item['select'].'">'.date_i18n('Y', $item['time']).'</a>';
	...
	*/
    ...
    function prepare_items( $args = array() ) {
		...
		$data = array();
		$years_all = $wppas_stats->get_stat_years();	
		...
		{
			$l = count($years_all)-1;
			$start = mktime(0,0,0, 1, 1, $years_all[0]);
			$end = mktime(23,59,59, 12, 31, $years_all[$l]);
			
			$args['stats'] = $wppas_stats->create_stats_array(array('start' => $sdate, 'end' => $edate));
		}
	
			
		$first_year = $years_all[0];
		$last_year = date('Y');
		
		foreach($years_all as $i => $year )
		{
			if( $year != $first_year )
			{
				$cnt = $wppas_stats->get_yearly_stats( 
					array(
						'stats'   => $args['stats'],
						'year'    => $year, 
						'type'    => $args['type']['slug'], 
				...
				$impressions = $wppas_stats->get_yearly_stats(
					array(
						'stats'   => $args['stats'],
						'year'    => $year,
						'type'    => 'impressions',
				...
				$clicks = $wppas_stats->get_yearly_stats(
					array(
						'stats'   => $args['stats'],
						'year'    => $year,
						'type'    => 'clicks',
				...
				$data[$i]['time'] = mktime(0,0,0,1,1, $year);
				$data[$i]['hits'] = $cnt;
